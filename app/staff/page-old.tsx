...
export default function StaffDashboard() {
  ...
  const [searchFilteredOrders, setSearchFilteredOrders] = useState<Order[]>([]); // Orders after search, before status filter
  ...
  const playNotificationSound = useCallback(() => {
    try {
      // Using Web Audio API to generate a simple notification sound
  ...
  useEffect(() => {
    ...
    if (orders.length > lastOrderCount && pendingOrders.length > 0) {
      ...
      // Play sound and show notification for the newest pending order
  ...
  useEffect(() => {
    ...
    // Save search-filtered results for count calculations
    setSearchFilteredOrders(searchFiltered);

    // Step 2: Apply status filter to search results
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="mt-2 text-base md:text-sm text-muted-foreground">
                ...
                {newOrdersCount > 0 && (
                  <span className="ml-2 inline-flex items-center rounded-full bg-primary px-2.5 py-0.5 text-xs font-bold text-primary-foreground">
                    {newOrdersCount} new
                  </span>
                )}
              </p>
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (e.target.value.trim() !== "" && newOrdersCount > 0) {
                  setNewOrdersCount(0);
                }
              }}
          ...
          <div className="relative custom-dropdown">
            ...
            >
              <div className="flex items-center justify-between">
                <span>
                  {selectedStatus} ({selectedStatus === "ALL" ? searchFilteredOrders.length : searchFilteredOrders.filter((order) => order.status === selectedStatus).length})
                </span>
                <svg
              ...
              <div className="absolute z-50 mt-2 w-full rounded-xl border-2 border-border bg-card shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="max-h-80 overflow-y-auto">
                  {statusFilters.map((status) => {
                    // Use searchFilteredOrders for counts (reflects search results)
        ...
        {filteredOrders.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-border bg-card py-12">
            ...
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className="mt-4 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90 transition-all"
              >
                Clear search
              </button>
            )}
